...
class PlaybackService extends ChangeNotifier {
  ...
  /// True while the live player is minimized into the floating "island"
  /// pill instead of `LiveIslandOverlay`'s own fullscreen presentation —
  /// playback keeps running exactly as before on the same controller;
  /// only the presentation changes. Set by `PlayerScreen`'s live-channel
  /// redirect (see that class's doc comment) whenever a *live* channel's
  /// fullscreen view is left, and cleared the moment fullscreen should be
  /// showing again for any reason (a fresh `play()` call, or that same
  /// redirect unconditionally calling `restore()`).
  ...
  Future<void> play(Channel channel, {bool silent = false}) async {
    ...
    // A silent cold-start resume starts *minimized*, never expanded —
    // `LiveIslandOverlay` shows its fullscreen presentation whenever
    // `isMinimized` is false, and this defaulting to `false` here used to
    // mean a silent resume's *own* grace-period timer clearing would, on
    // its own with no user interaction at all, auto-jump straight to
    // fullscreen the moment it elapsed — a different flavor of the exact
    // cold-start auto-jump bug this silent-resume mechanism exists to
    // prevent. Starting minimized means the grace period elapsing just
    // reveals the small pill (matching the phone layout's MiniPlayerBar:
    // "show what's playing" once it's safe to), never the fullscreen
    // chrome — that only ever happens from a deliberate tap/expand.
    isMinimized = silent;
