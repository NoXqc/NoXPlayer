...
class _SeriesDetailScreenState extends State<SeriesDetailScreen> {
  late Future<({Map<int, List<Channel>> episodes, String? plot})>
      _episodesFuture;
  ...
  FocusNode _firstFocusNodeForSeason(int season) => _seasonFirstFocusNodes
      .putIfAbsent(season, () => FocusNode(debugLabel: 'season-$season-first'));
    ...
    _episodesFuture =
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (seasons.isNotEmpty &&
            (result.episodes[seasons.first]?.isNotEmpty ?? false)) {
  ...
  Widget build(BuildContext context) {
    ...
    return withTvThemeIfNeeded(
        ...
        (context) => Scaffold(
              ...
              body: FutureBuilder<
                  ({Map<int, List<Channel>> episodes, String? plot})>(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, j) {
                                  ...
                                  return PosterCard(
                                    ...
                                    // — never a blank card.
                                    imageUrl: (episode.logoUrl != null &&
                                            episode.logoUrl!.isNotEmpty)
                                        ? episode.logoUrl
                                        : widget.series.coverUrl,
                                    watched: storage.isFullyWatched(episode.id),
                                    progressFraction:
                                        storage.getWatchedFraction(episode.id),
                                    focusNode: j == 0
                                        ? _firstFocusNodeForSeason(season)
                                        : null,
                                    onTap: () => _openEpisode(
                                        episode, allEpisodesInOrder),
                                    onFocusGained: () {},
                                  );
                                },
                              ),
